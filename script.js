...
function run(){
    ...
    async function predict(position) {
        ...
        var num=parseFloat(result.toString().slice(14,23));
        if(arr3[4]==1)
        {
            document.getElementById("output").innerHTML="Yield of "+arr[2].slice(5)+" is : "+Math.round((num + Number.EPSILON) * 100) / 100+" (Bales/Hectare)";
        }
        else
        {
            document.getElementById("output").innerHTML="Yield of "+arr[2].slice(5)+" is : "+Math.round((num + Number.EPSILON) * 100) / 100+" (Tonne/Hectare)";
        }
    }
}